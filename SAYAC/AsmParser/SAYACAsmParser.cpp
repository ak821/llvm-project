//===-- SAYACAsmParser.cpp - Parse SAYAC assembly to MCInst instructions ----===//
//
...
class SAYACOperand : public MCParsedAsmOperand {
  enum OperandKind {
    // KindInvalid,
    OpKind_Token,
    OpKind_Reg,
...
public:
  SAYACOperand(OperandKind kind, SMLoc startLoc, SMLoc endLoc)
  ...
  static std::unique_ptr<SAYACOperand> createToken(StringRef Str, SMLoc Loc) {
    auto Op = std::make_unique<SAYACOperand>(OpKind_Token, Loc, Loc);
    Op->Token.Data = Str.data();
    Op->Token.Length = Str.size();
    return Op;
  }

  static std::unique_ptr<SAYACOperand> createReg(unsigned Num, SMLoc StartLoc,
                                                SMLoc EndLoc) {
    auto Op = std::make_unique<SAYACOperand>(OpKind_Reg, StartLoc, EndLoc);
    Op->Reg.RegNo = Num;
    return Op;
  }

  static std::unique_ptr<SAYACOperand> createImm(const MCExpr *Expr,
                                                SMLoc StartLoc, SMLoc EndLoc) {
    auto Op = std::make_unique<SAYACOperand>(OpKind_Imm, StartLoc, EndLoc);
    Op->Imm = Expr;
    return Op;
  }
...
class SAYACAsmParser : public MCTargetAsmParser {
// Auto-generated instruction matching functions
#define GET_ASSEMBLER_HEADER
#include "SAYACGenAsmMatcher.inc"
  ...
  OperandMatchResultTy parsePCRel10(OperandVector &Operands) {
    return MatchOperand_ParseFail;
  }
...
public:
  SAYACAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
bool SAYACAsmParser::ParseDirective(AsmToken DirectiveID) { return true; }

bool SAYACAsmParser::ParseInstruction(ParseInstructionInfo &Info, StringRef Name,
...
bool SAYACAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  if (Lexer.is(AsmToken::Percent)) {
    ...
    if (ParseRegister(RegNo, StartLoc, EndLoc))
      return true;
    Operands.push_back(SAYACOperand::createReg(RegNo, StartLoc, EndLoc));
    return false;
  }
  ...
  if (Lexer.is(AsmToken::Integer)) {
    ...
    Operands.push_back(SAYACOperand::createImm(Expr, StartLoc, EndLoc));
    return false;
  }
...
bool SAYACAsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
...
OperandMatchResultTy SAYACAsmParser::tryParseRegister(unsigned &RegNo,
                                                     SMLoc &StartLoc,
                                                     SMLoc &EndLoc) {
  bool Result =
...
bool SAYACAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      // TODO
      // ErrorLoc = ((SAYACOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
  ...
  case Match_MnemonicFail: {
    ...
    SAYACOperand &Op = static_cast<SAYACOperand &>(*Operands[0]);
    std::string Suggestion = SAYACMnemonicSpellCheck(Op.getToken(), FBS, 0);
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeSAYACAsmParser() {
  RegisterMCAsmParser<SAYACAsmParser> x(getTheSAYACTarget());
}